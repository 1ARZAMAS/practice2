...
int EilerTheorem(int p) { // определяем количество взаимно простых чисел для p 
    int result = p;
...
int main(){
    ...
    while (Prime(modP) == false){
        cout << "Число должно быть простым!\nПопробуйте еще раз: ";
        cin >> modP;
    }
    
    if (basis < modP && power == (modP - 1)){ // Воспользуемся теоремой Ферма
    ...
    } else if(NOD(basis, modP) == 1){ // если числа взаимнопростые, можем воспользоваться теореой Эйлера
        if (power == EilerTheorem(modP)){
            cout << "Согласно теореме Эйлера получим: " << basis << "^" << power << " mod " << modP << " = 1" <<endl;
            pair<int, int> ost = remainder(basis, power, modP);  //рассматриваем остатки через функцию
            cout << "Проверим, используя логарифм: " << ost.second << endl;
        } 
    } else{
            power == EilerTheorem(modP);
